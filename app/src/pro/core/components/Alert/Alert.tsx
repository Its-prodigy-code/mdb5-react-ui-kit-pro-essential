...
const MDBAlert: React.FC<AlertProps> = ({
  ...
  useEffect(() => {
    if (position) {
      ...
      if (positionX === 'center') {
        setPlacement({
          [positionY]: `${offset && offset + verticalOffset}px`,
          left: '50%',
          [oppositeY]: 'unset',
    ...
    if (stacking) {
      if (isShown) {
        ...
        if (selectedAlerts.length > 0) {
          ...
          if (posY === 'top') {
            offsetValue = (selectedAlerts[selectedAlerts.length - 1] as HTMLElement).style.top;
          } else {
            offsetValue = (selectedAlerts[selectedAlerts.length - 1] as HTMLElement).style.bottom;
          }

          const parsedValue = parseInt(offsetValue.split('px')[0]);

          if (offset) {
            vOffset = parsedValue + offset;
          } else {
            vOffset = parsedValue;
          }
        }
...
MDBAlert.defaultProps = { offset: 10, width: 'width', delay: 1000, show: false, stacking: false };
