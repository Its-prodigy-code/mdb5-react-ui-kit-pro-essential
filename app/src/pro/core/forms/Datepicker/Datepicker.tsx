...
const MDBDatepicker: React.FC<DatepickerProps> = ({
      ...
      if (focused && tabCount !== 4) {
        if (view === 0) {
          switch (e.key) {
            ...
            case ENTER:
              !isDateDisabled(activeDate, min as Date, max as Date, filter) && setSelectedDate(activeDate);
              break;
          ...
          switch (e.key) {
            ...
            case PAGE_DOWN:
              ...
              break;
            case ENTER:
              !isYearDisabled(getYear(activeDate), min as Date, max as Date) && setView(2);
              break;
            case SPACE:
              !isYearDisabled(getYear(activeDate), min as Date, max as Date) && setSelectedDate(activeDate);
              return;
            default:
              return;
          }
          ...
          switch (e.key) {
            ...
            case PAGE_DOWN:
              ...
              break;
            case ENTER:
              !isDateDisabled(activeDate, min as Date, max as Date, filter) && setView(0);
              break;
            case SPACE:
              !isDateDisabled(activeDate, min as Date, max as Date, filter) && setSelectedDate(activeDate);
              return;
  ...
  const confirmSelection = () => {
    if (format && selectedDate) {
      const dateString = formatDate(
        selectedDate,
        format,
        weekdaysShort as Array<string>,
        weekdaysFull as Array<string>,
        monthsShort as Array<string>,
        monthsFull as Array<string>
      );
  ...
  const setActiveDay = (date: Date) => {
    const isNotDisabled = !isDateDisabled(date, min as Date, max as Date, filter);
  ...
  useEffect(() => {
    if (defaultValue.current) {
      ...
      const date = parseDate(value, format, monthsFull as Array<string>, monthsShort as Array<string>);
      setValue(value);
  ...
  }, [defaultValue, value, format, setValue, monthsFull, monthsShort]);
  ...
  const handleArrows = (flag: boolean) => {
    if (view === 0) {
      ...
      if (flag) {
        !isNextDateDisabled(addMonths(date, -1), 'days', 1, min as Date, max as Date) && setActiveDate(date);
      } else {
        !isPreviousDateDisabled(addMonths(date, 1), 'days', 1, min as Date, max as Date) && setActiveDate(date);
      }
    } else if (view === 1) {
      const date = flag
  ...
  const handleInput = (e: any) => {
    const date = parseDate(e.target.value, format, monthsFull as Array<string>, monthsShort as Array<string>);
  ...
  const dayView = (
    <table className='datepicker-table'>
      ...
      <tbody className='datepicker-table-body'>
        {getDatesArray(activeDate, selectedDate, min as Date, max as Date, filter, startDay as number).map(
          (week, weekNr) => (
            <tr key={weekNr}>
              {week.map((day, dayNr) => (
                <td
  ...
  const monthView = (
    <table className='datepicker-table'>
      <tbody className='datepicker-table-body'>
        {getMonthsArray(monthsShort as Array<string>).map((row, i) => (
          <tr key={i}>
            {row.map((month) => (
              <td
                ...
                className={clsx(
                  ...
                  getMonth(getToday()) === monthsShort.indexOf(month) &&
                    ...
                    'current',
                  isMonthDisabled(monthsShort.indexOf(month), getYear(activeDate), min as Date, max as Date) &&
                    'disabled'
                )}
              >
            ...
            {row.map((year) => (
              <td
                ...
                className={clsx(
                  ...
                  isYearDisabled(year, min as Date, max as Date) && 'disabled',
