...
const MDBSelect: React.FC<SelectProps> = ({
  ...
  useEffect(() => {
    getValue && getValue(multiple ? textMultiple : inputValue);
  }, [inputValue, textMultiple, multiple, getValue]);
  ...
  useEffect(() => {
    ...
    if (isFoucsed && !attachELements && arrowFocus !== undefined) {
      if (!multiple) {
        const mappedFocusData = data.map((e: mainDataInf, index: number) => {
          if (index === arrowFocus) {
  ...
  const handleListClick = useCallback(
    ...
    [data, filteredData, getData, multiple, textMultiple, getSelectedValue]
  );
